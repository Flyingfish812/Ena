...
## 九、基于尺度相关性的可恢复尺度判定（v2.1）

本章讨论一种**不依赖误差幅值、而直接刻画结构一致性与信息保留能力的尺度判定方法**。该方法以频域尺度相关性为核心，并在 v2.1 中与频域相干性（coherence）与信噪比（SNR）统一于同一分析框架下。

---

### 9.1 频域尺度相关系数（Spectral Correlation）

对每个时间帧 $t)，在径向波数 bin $\Omega_m$ 上，定义预测场与真实场在频域的尺度相关系数：
...
* $\hat x_t(p,q)$ 为真实场的二维傅里叶系数；
* $\widehat{\hat x}_t(p,q)$ 为预测场的二维傅里叶系数；
* $\Re(\cdot)$ 表示取实部。
...
* $\rho(k)<0$：预测与真实场出现系统性反相或相位翻转。

**重要说明**：
尺度相关系数 $\rho(k)$ 对幅值缩放不敏感，主要刻画预测与真实场在该尺度上的**线性结构一致性**。

---

### 9.2 与频域相干性的关系（v2.1 定位修订）

在 v2.1 中，引入了频域相干性（magnitude-squared coherence）作为更标准、审稿友好的结构一致性度量。二者的关系如下：

对单帧 $t$，定义交叉谱与功率谱：
$$
C_{tp,t}(p,q)=\widehat{\hat x}*t(p,q),\overline{\hat x_t(p,q)},\quad
P_x(p,q)=|\hat x_t(p,q)|^2,\quad
P*{\hat x}(p,q)=|\widehat{\hat x}_t(p,q)|^2.
$$
...
\frac{|C_{tp,t}(p,q)|^2}{P_x(p,q),P_{\hat x}(p,q)+\varepsilon}.
$$

对径向 bin 取平均并对时间统计，可得：
$$
\mathrm{coh}(k_m)
=
...
* $\rho(k)$ 是 **带符号的线性相关系数**（保留相位符号信息）；
* $\mathrm{coh}(k)$ 是 **无符号的结构一致性强度**（相位一致性平方归一化）。

在无系统性反相的情况下，二者在定性判定上高度一致；
在存在相位翻转或局部反相时，(\rho(k)$ 提供额外诊断信息。

---

### 9.3 可恢复尺度截止的定义（Adaptive Spectral Cutoff, v2.1）

基于尺度相关性或相干性，引入**可恢复尺度截止波数**的统一定义。

#### （一）基于尺度相关系数的定义

给定相关性阈值 $\delta_\rho>0)，定义：
...
\max\left{
k_m ;\middle|;
|\rho(k_m)| \ge \delta_\rho
\right}.
$$

#### （二）基于频域相干性的定义（推荐）

给定相干阈值 $\delta_{\mathrm{coh}}\in(0,1)$，定义：

$$
k_{\mathrm{eff}}^{(\mathrm{coh})}
=

\max\left{
k_m ;\middle|;
\mathrm{coh}(k_m) \ge \delta_{\mathrm{coh}}
\right}.
$$

对应的最小可恢复物理尺度为：
$$
\ell_{\mathrm{eff}}=\frac{1}{k_{\mathrm{eff}}},
\qquad
s_{\mathrm{eff}}=\frac{\ell_{\mathrm{eff}}}{D},
$$
其中 $D$ 为特征几何尺度（如障碍物直径或通道高度）。

**判定原则**：
当波数超过 $k_{\mathrm{eff}}$ 后，预测结果在该尺度上已不再与真实物理场保持可区分的结构一致性。

---

### 9.4 与误差型尺度指标的关系（统一视角）

需要强调的是，尺度相关/相干型指标与误差型指标刻画的是**不同层面的模型能力**：

* $\mathrm{NRMSE}*{\mathrm{F}}(k)$、累计 $\mathrm{NRMSE}*{\le K}$：
  描述的是误差幅值随尺度的累积行为；
* $\rho(k)$、$\mathrm{coh}(k)$：
  描述的是预测与真实场在该尺度上的结构一致性；
* 二者在高频区间可能给出完全不同的判断。

在本项目的 v2.1 框架中：

* **尺度相关性 / 相干性指标作为可恢复尺度判定的主指标**；
* 误差型指标仅作为辅助诊断量，用于解释误差来源与能量分布。

---

### 9.5 本章结论（可直接用于论文表述）

> 当整体预测误差受噪声水平或模型容量限制而进入平台期时，
> 单一误差指标已不足以刻画模型的多尺度预测能力。
>
> 通过引入基于频域尺度相关性与相干性的判定方法，可以在误差水平近似不变的情况下，
> 仍然定量地区分模型在不同空间尺度上的结构信息保留能力，
> 从而为“可恢复尺度”的定义提供更具物理意义与统计稳定性的判据。